...
#'
#'
create_and_run_job <- function(name = "R Job", notebook_path, job_config = "default",
                               workspace, token = NULL) {
  
  job <- create_job(name = name, notebook_path = notebook_path,
                    job_config = job_config, workspace = workspace, token = token,
                    verbose = F)

  ## Run using job_id from create_job()
  run <- run_job(job_id = job$job_id[[1]],
                 workspace = workspace, token = token, verbose = F)
